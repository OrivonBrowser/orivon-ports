...
export async function recon (clone: string): Promise<ReconReport> {
  ...
  // Cross-check the preload against what the renderer actually calls. Neither
  // source is provably complete -- computed keys and Object.assign defeat both
  // -- so the union of the two is a floor, never a total.
  const members = new Set<string>()
  for (const name of bridgeNames) {
    const call = new RegExp(`window\\.${name}\\.([A-Za-z0-9_$]+)`, 'g')
    for (const file of rendererFiles) {
      const text = await readFile(file, 'utf8').catch(() => '')
      for (const match of text.matchAll(call)) members.add(match[1] as string)
    }
  }
