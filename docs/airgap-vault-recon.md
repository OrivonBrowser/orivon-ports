...
| WebAssembly compilation, blob workers, inline scripts/styles | Not blocked on the dev-serving path this port runs on (no CSP is set there), nor on the installed/cached path, whose CSP (`orivon-mvp/src/loader/serve/csp.ts`) carries `'wasm-unsafe-eval'` and `worker-src 'self' blob:` | Nothing |
